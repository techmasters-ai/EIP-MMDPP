...
def _collect_batch_evidence_text(docling_document_json: dict[str, Any]) -> str:
    parts: list[str] = []
...
def _identity_is_supported_by_batch_text(identity_value: Any, evidence_text: str) -> bool:
    if not isinstance(identity_value, str):
        return False
    normalized = _normalize_evidence_text(identity_value)
...
def _filter_pass_output_by_batch_text(
    ...
    for field_name, field_info in template_cls.model_fields.items():
        ...
        for item in raw_items:
            ...
            if _identity_is_supported_by_batch_text(primary_identity, evidence_text):
                kept_items.append(item)
...
def _filter_provenance_rows_by_allowed_identities(
