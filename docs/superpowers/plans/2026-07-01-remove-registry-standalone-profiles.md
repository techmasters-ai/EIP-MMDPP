...
**Acceptance Criteria:**
- [ ] `governance.query_profiles` exists with columns: `id` (uuid pk — internal only), **`profile_key` (str, UNIQUE — the existing `QueryProfileDefinition.id` string key; this is what dossier `section_profile_ids` and search `profile_id` reference)**, **`label` (str — display name, = existing `QueryProfileDefinition.label`)**, `description`, `kind` (str), `root_entity_types` (JSONB), `definition` (JSONB — the rest of the profile body: `target_entity_types`, `traversals`, `section_profile_ids`, `profile_sections`, `profile_subgroup`, `include_associated_systems`), `source_id` (uuid FK `ingest.sources` ON DELETE SET NULL, nullable), `enabled` (bool default true), `created_by`, `created_at`, `updated_at`.
  - **Rationale (review finding 1):** existing profiles are keyed by a *string* `id` + a `label` (`schemas/query_profiles.py:90-91`), NOT a `name`. `profile_key` preserves that stable string key so cross-references (dossier→section, search `profile_id`) keep working; the uuid `id` is DB-internal.
- [ ] The migration **deterministically** picks the source registry: `WHERE is_active IS TRUE ORDER BY updated_at DESC LIMIT 1`, else the latest by `updated_at`; it copies that row's 4 `profiles` into `query_profiles` (`source_id` NULL, `enabled` TRUE), mapping embedded `id`→`profile_key`, `label`→`label`, `kind`, `root_entity_types`, rest→`definition`.
- [ ] The migration **asserts exactly 4 profiles** were migrated; if the registry table is empty / has no profiles, it **falls back to a literal copy of the 4 canonical profiles embedded in the migration** (copy the dicts from `alembic/versions/0018_starter_profiles_to_section_properties.py::NEW_PROFILES` — they're already literal there). Never silently seed zero.
- [ ] Downgrade drops the table (data loss acceptable — it's derived).

**Verify:** `alembic upgrade head` then `docker exec eip-mmdpp-postgres-1 psql -U eip -d eip -tAc "SELECT profile_key,kind,source_id,enabled FROM governance.query_profiles ORDER BY profile_key"` → 4 rows (`system_components`, `system_dossier`, `system_performance`, `system_rf_parameters`), all `source_id` NULL / `enabled` t.
...
- [ ] `resolve_root_entity(...)` accepts an optional `source_id`; when set, **each candidate set (alias, fulltext, direct, co-extracted) is filtered by source BEFORE `_select_best_candidate`** — NOT after (review finding 3: post-filtering the single global winner could hide a valid in-source candidate). A candidate qualifies if it has an `EXTRACTED_FROM` chunk whose document's `source_id` matches.
- [ ] `attach_evidence`/`_fetch_chunk_evidence` accept `source_id`; when set, the predicate **`AND d.source_id = :source_id`** is added to **BOTH `UNION ALL` branches** (text_chunks + image_chunks) — the query aliases documents as `d` (`query_profiles.py:697` text branch, `:723` image branch), NOT `documents` (review finding 4).
- [ ] When source-scoped, `include_associated_systems` results from `get_associated_systems(...)` (`query_profiles.py:806`) are **also filtered to the selected source** (their related entities must belong to that source), not just their evidence (review finding 5).
- [ ] With `source_id=None` behavior is byte-identical to today (Global = unfiltered) on ALL three paths.

**Verify:** `python3 -m pytest tests/unit/test_query_profile_source_scope.py -v` — asserts (a) candidate filtering runs before selection, (b) the evidence predicate is present on both UNION branches only when scoped, (c) associated systems are source-filtered when scoped, and none of it fires when Global.

**Steps:**
- [ ] **Step 1 (resolve):** Read `resolve_root_entity :544` + `_select_best_candidate`. For each candidate-producing path (alias/fulltext/direct/co-extracted), when `source_id` is set, filter that path's candidate list to entities with an in-source document BEFORE ranking/selection. Keep the None path unchanged.
...
- [ ] New/kept routes operate on profiles directly: `GET /v1/query-profiles` (list all, `?enabled_only=`), `POST /v1/query-profiles` (create), `GET/PUT/DELETE /v1/query-profiles/{profile_key}`, `POST /v1/query-profiles/search/section`, `POST /v1/query-profiles/search/dossier`. Each create/update accepts `source_id` (nullable) + `enabled`.
- [ ] **The external identifier is `profile_key`** (the existing string id), NOT the uuid — search requests' existing `profile_id: str` field (`schemas:175`) resolves to `profile_key`, and dossier `section_profile_ids` reference `profile_key`, so existing cross-references keep working (review finding 1). The uuid `id` is DB-internal.
- [ ] `search/section` + `search/dossier` load the profile by `profile_key` and honor its `source_id`.
- [ ] Schemas drop `QueryProfileRegistryCreate/Update/Response`; add `QueryProfileCreate/Update/Response` (incl. `profile_key`, `label`, `source_id`, `enabled`). Keep `validate_shape` + the RADAR/MISSILE root frozenset.
...
- [ ] E2E: create a profile scoped to a specific source → `search/section` returns only entities/evidence from that source's documents, **and any `include_associated_systems` results are also confined to that source**; the same profile as Global returns the unfiltered superset (scoped result set ⊆ global).
- [ ] The 4 migrated profiles run via `search/section`/`search/dossier` without error, referenced by `profile_key`.
