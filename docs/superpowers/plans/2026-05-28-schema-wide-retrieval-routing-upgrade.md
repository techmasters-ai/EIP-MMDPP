...
- **Index-time unit convention must stay pass-agnostic** — `emit_unit_hint=True` at index time (pre-`258bb9a`) imposed a static SI-unit preamble that overrode column-header units and broke extraction when a pass schema expected different units (km vs m), cutting recall (radar_power_rf 8/22, missile_kinematics 10/16). Fixed by `emit_unit_hint=False` upstream (header units verbatim). Do NOT re-enable unit hints in the pass-agnostic index path; only docling-graph's per-pass path (which has pass context) may emit them.
