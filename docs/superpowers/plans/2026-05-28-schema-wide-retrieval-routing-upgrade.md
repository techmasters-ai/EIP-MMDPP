...
**Empirical decomposition (run `f4e03e7e`, 2026-05-29 — wire-off, COMPLETE, 447.8m vs baseline 228.5m):** the ~2× wall was **NOT chunk count or size**. Per-batch decomposition (`diagnostics_json.batch_timings`) showed the excess came from a handful of LLM batches hitting the **1800s stream-wall-timeout + retry under intermittent pool contention** — **9 batches >1500s vs 0 in baseline** (max 3675s); for `missile_kinematics`, **3 of 20 batches = 75% of the pass wall**. The honest wire-off chunk-count cost is only ~2× (clean `radar_power_rf`: 13→22 chunks, 13.5→30.9m, **0 stalls**) — exactly what Phase C targets. **Chunk sizes are small** (~270-430 tok/chunk, cap 512), not a factor. Full root-cause + the (out-of-scope) infra follow-ups — timeout policy, pool scheduling, GPU parallelism, identity-pass wall — are tracked in **`docs/operational/walltime-root-cause-and-followups.md`**.
...
**One new worker env var (`166dac2`).** The plan's *retrieval* behavior keys off existing `VECTOR_ROUTER_MODE` + `EXTRACTION_INDEX_MODE`; new retrieval defaults live in `RetrievalProfile` config + manifest YAML. But `166dac2` added a new worker env var **`EXTRACTION_INDEX_UPSTREAM_TABLE_NORM`** — a kill-switch for the upstream table-norm (present in both `.env`/`.env.example`, default `true`, but **`false` in this worktree's live `.env`** for an SA-2 recall A/B). Upstream table-aware indexing now requires THREE flags all on: `DOCLING_GRAPH_TABLE_NORMALIZATION_ENABLED` (master, default-off in `.env.example`), `EXTRACTION_INDEX_UPSTREAM_TABLE_NORM` (worker gate, default-on but currently OFF here), and — for D3's `content_type` marking — `DOCLING_GRAPH_SUPPRESS_RAW_TABLE_MARKDOWN` (default-on). In code, `_norm_on = is_table_normalization_enabled_graph() AND _upstream_norm_on` (`extraction_chunk_index.py:~1153`).
...
- **Table normalization is applied UPSTREAM in `build_extraction_index_hybrid` (DONE in `ae5f501`)**, NOT via `hybrid_chunking.py`. Before HybridChunker, the index builder runs `web_cruft_sanitizer.sanitize_docling_document` (always) then `normalize_tables` + `render_for_graph(..., emit_unit_hint=False)` (synthesizing per-row TextItems for ALL tables; `emit_unit_hint` was flipped to `False` in `258bb9a` — see D2), gated on the pre-existing **default-off** `DOCLING_GRAPH_TABLE_NORMALIZATION_ENABLED`. This **supersedes** the original `_substitute_table_chunks`/`_NormalizedTableChunkAdapter` approach — there are no adapter chunks in this path.
...
**Status: implemented, but UNDER A/B and currently DISABLED in the live `.env` (`166dac2`).** `build_extraction_index_hybrid` (`app/services/extraction_chunk_index.py:1136-1222`) sanitizes + normalizes tables upstream of HybridChunker, synthesizing per-row TextItems via `normalize_tables` + `render_for_graph`. `MergedChunk` was intentionally NOT extended — synthesized rows flow through the chunker as ordinary text. The original plan's `hybrid_chunking.py` / `_substitute_table_chunks` / adapter-branch approach is **superseded** and not needed. **Caveat:** upstream norm is now gated by `EXTRACTION_INDEX_UPSTREAM_TABLE_NORM` (`166dac2`) in addition to the master flag, and is set **`false`** here because the per-row sentence format (column-header units like `Max Range: 29000 m`) is the suspected SA-2 recall regression — `emit_unit_hint=False` removed only the SI preamble, not the per-row column-unit suffix. **If the A/B confirms the per-row format is the culprit, the upstream approach reverts to raw-markdown tables — which re-scopes Phase D and removes D3's synth-row basis (no `#/texts/M` rows → nothing for D3 to mark).** Settle this A/B before building D3.

**Verify-only:**
- [ ] Confirm BOTH `DOCLING_GRAPH_TABLE_NORMALIZATION_ENABLED=true` (master; default-off in `.env.example`, `true` in live `.env`) AND `EXTRACTION_INDEX_UPSTREAM_TABLE_NORM=true` (worker gate; default-on but **`false` in live `.env`** for the current A/B) — upstream norm only runs when both are on.
...
**Files:**
- Reuse: `app/services/table_normalization/` helpers — the upstream path uses `render_for_graph` (`render_graph.py`) with **`emit_unit_hint=False`** (set in `258bb9a`), NOT `render_for_embedding`. Index-time is pass-agnostic, so the static SI-unit preamble must be suppressed: it was overriding the original column-header units (e.g. forcing `Min Range: 8000 m` when `missile_kinematics.min_intercept_km` expects km), which cut recall (radar_power_rf 8/22, missile_kinematics 10/16). docling-graph's per-pass path keeps `emit_unit_hint=True` because it has pass context.

**Steps:**
- [ ] Add tests around the upstream normalized render output: each emitted row chunk must include table/header context, row labels, values, and **the column-header units verbatim** (NOT an SI preamble) — e.g. assert a `Max Range km` header yields `Max Range km: 75` in the row, with `km` preserved.
...
- **Index-time unit convention / the whole upstream-table-norm approach is under A/B** — `emit_unit_hint=True` at index time (pre-`258bb9a`) imposed a static SI-unit preamble that overrode column-header units and cut recall (radar_power_rf 8/22, missile_kinematics 10/16). `258bb9a` removed the preamble (`emit_unit_hint=False`), but the **per-row column-unit suffix** (e.g. `Max Range: 29000 m`) may *itself* be the regression. `166dac2` added a worker kill-switch `EXTRACTION_INDEX_UPSTREAM_TABLE_NORM` (currently **`false`** here) to A/B with upstream norm fully off (raw markdown tables verbatim). **Open outcome:** if raw-markdown wins, Phase D's upstream synthesis is reverted and D3 loses its synth-row basis (re-scope Phase D); if normalized-rows win once unit handling is fixed, Phase D proceeds as written. Do NOT re-enable index-time unit hints; only docling-graph's per-pass path (which has pass context) may emit them.
...
- **Wall measurements are contention-confounded (acceptance #3)** — the 1800s `OLLAMA_STREAM_WALL_TIMEOUT` kill+retry amplifies intermittent `gemma4:31b` pool contention into 2000-3675s per-batch stalls (run `f4e03e7e`: **9 batches >1500s vs 0 in baseline**; 3 = 75% of `missile_kinematics`' wall). A wall regression measured on a contended pool may be an infra artifact, **not** the retrieval path — **re-measure on an idle pool before failing acceptance #3.** The honest wire-off chunk cost is ~2× (clean `radar_power_rf`). Root cause + infra follow-ups (timeout policy, pool scheduling, GPU parallelism, identity-pass wall) are out of scope and tracked in `docs/operational/walltime-root-cause-and-followups.md`.
