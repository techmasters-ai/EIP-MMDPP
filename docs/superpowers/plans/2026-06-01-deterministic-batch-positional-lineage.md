...
**Acceptance Criteria:**
- [ ] Field: each `_field_evidence` row's `chunk_id` is RESOLVED from its `element_uid`/`self_refs` via the same self_ref→chunk resolver as Task 4 (no longer hardcoded `None`); `page` retained. **PHASE CONSTRAINT (verified):** `_field_evidence` is serialized in `_import_graph_phase_nodes` (pipeline.py:1303, the MERGE phase), but `identity_map` + `element_uid_chunk_map` are built only in `derive_structure_links` (pipeline.py:9296+, a SEPARATE later task) — the resolver maps are NOT in scope at 1303. So field resolution MUST happen in one of: (a) build the identity_map + element_uid_chunk_map in the merge phase too (reuse `_load_identity_map(document_id)` + an `element_uid_chunk_map` query) and resolve `row.chunk_id` before serializing at 1303; OR (b) leave `chunk_id=None` at 1303 and resolve `_field_evidence` chunk_ids in `derive_structure_links` by re-reading + UPDATE-ing each committed entity vertex's `_field_evidence` property where the maps already exist. Pick (a) — resolve at the merge phase using `_load_identity_map` (already a module function) + a one-shot `element_uid_chunk_map` build, so `_field_evidence` is written correct once. A field whose `element_uid` resolves to chunk C has `chunk_id == C`.
